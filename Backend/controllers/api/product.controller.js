  ...
  try {
    ...
    const params = {
      ...
      txt_search: request.query.txt_search
        ...
        : "",
      categoryInfo: request.query.categoryInfo
        ? request.query.categoryInfo.trim()
        : "",
    ...
    for (let i = 0; i < queryResult.rows.length; i++) {
      ...
      if (element.dataValues.discount > 0)
        element.dataValues.salePrice =
          Math.ceil((element.price * (100 - element.discount)) / 100000) * 1000;
      else element.dataValues.salePrice = element.dataValues.price;

      const queryProductDetail = await getDetailByProductById(
        element.dataValues.id,
      );
      const queryProductImage = await getImageByProductById(
        element.dataValues.id,
      );
...
async function showProductInfo(request, response) {
  try {
    ...
    const Result = {
      ...
      productDetails: queryProductDetail.rows,
      productImages: queryProductImage.rows,
    };
...
async function create(request, response) {
  try {
    ...
    for (var i = 0; i < details.length; i++) {
      ...
      const validateResponseDetail =
        validators.validateProductDetail(newProductDetail);
    ...
    if (dbProduct) {
      const updateProduct = {
        ...
        quan_sold: request.body.quan_sold
          ? request.body.quan_sold
          : dbProduct.quan_sold,
        quan_in_stock: request.body.quan_in_stock
          ? request.body.quan_in_stock
          : dbProduct.quan_in_stock,
        origin: request.body.origin ? request.body.origin : dbProduct.origin,
        fromCity: request.body.fromCity
          ? request.body.fromCity
          : dbProduct.fromCity,
      ...
      const validateResponse = validators.validateProduct(updateProduct);
