...
function Cart() {
      ...
      cart.map((item) => {
        ...
        return id === item.id
          ? {
              ...
              total_price: item.total_price + pricee_item,
            }
          : item;
      }),
    );
  ...
  const handleChange = (item, event) => {
    if (event.target.checked) {
      ...
      setOrderedItems((cart) => cart.filter((i) => i.id !== item.id));
      setTotalCost((total) => total - parseInt(item.total_price));
    }
  };
    ...
    <div className="cart-container">
      <div className="cart-header">
        <div className="cart-header-logo">
          <Link to={"/homepage"}>
            <img
              src={shopeeLogo}
              alt="anh logo shopee"
              className="cart-header-logo_img"
            />
          </Link>
        ...
        <div className="cart-header-search">
          <input
            type="text"
            placeholder="Siêu Vocher cùng săn ngay"
            className="cart-header-inp"
          />
            ...
            <table>
              <tr>
                <td colSpan={2} className="cart-product-col-double">
                  <input
                    type="checkbox"
                    id="cart-total-product"
                    name="cart-total-product"
                  ...
                  <div className="cart-product-wrap">
                    <table>
                      <tr>
                        <td className="cart-product-col-special">
                          <div className="cart-product-des">
                            <input
                              ...
                              name={cart.id}
                              className="cart-product-checkbox"
                              onChange={(event) => handleChange(cart, event)}
                            />
                            <img
                              src={
                                cart.ProductDetail.Product.ProductImages[0]
                                  .image
                              }
                              alt="anh"
                        ...
                        <td
                          className="cart-product-col-single"
                          style={{ color: "#ee4d2d" }}
                        >
                          {" "}
                    ...
                    <div className="cart-product-shop">
                      <FontAwesomeIcon
                        icon={faMap}
                        className="cart-product-shop-voucher"
                      />
                    ...
                    <div className="cart-product-ship">
                      <img
                        src={freeship}
                        alt="anh"
                        className="cart-product-voucher"
                      />
                      <p>
                        Giảm ₫15.000 phí vận chuyển đơn tối thiểu ₫50.000; Giảm
                        ₫25.000 phí vận chuyển đơn tối thiểu ₫150.000
                        <span style={{ color: "#007ac8", marginLeft: "12px" }}>
                          Tìm hiểu thêm
                        </span>
                      </p>
                    </div>
