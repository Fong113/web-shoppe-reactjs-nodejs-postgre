      ...
      <table className="admin-product-list">
        <tr key={item.id}>
          ...
          <td className="admin-product-detail">
            <button
              type="button"
              className="admin-product-see-detail"
    ...
    return (
      <table className="admin-product-list">
        <tr>
          ...
          <td className="admin-product-update">
            <button className="admin-product-btn">Cập nhập</button>
          </td>
          <td className="admin-product-delete">
  ...
  function handleValues(e) {
    ...
    const addProduct = async (params) => {
      try {
        ...
        await axiosClient.post("/api/product/addPictures", formData, {
          headers: {
  ...
  return (
    <div className="admin-product-wrap">
      <div className="admin-product-add">
        <button className="admin-product-add-button" onClick={handleAdd}>
          Thêm sản phẩm
        </button>
        ...
        >
          Thêm sản phẩm ( phiên bản 2){" "}
        </button>
        {show2 && <AddProduct />}
        <form onSubmit={handleValues2}>
          {show ? (
            <>
              <label for="title" className="admin-product-label">
                Nhập tên sản phẩm
              </label>
              <input
                type="text"
                placeholder="Nhập tên sản phẩm"
                className="admin-product-add-name"
                name="title"
                id="title"
                onChange={(e) => setTitle(e.target.value)}
                value={title}
              />
              <label for="quantity" className="admin-product-label">
                Nhập số lượng sản phẩm
              </label>
              <input
                type="text"
                className="admin-product-add-quantity"
                name="quantity"
                id="quantity"
                value={quantity}
              ...
              />
              <label for="price" className="admin-product-label">
                Nhập số tiền
              </label>
              <input
                type="number"
                placeholder="Nhập số tiền (đơn vị đđ)"
                name="price"
                id="price"
              ...
              >
                <input {...getInputProps()} />
                {isDragActive ? (
                  <p className="admin-product-img-des">
                ...
                <ul className="admin-product-img-list">
                  {files.map((file, index) => (
                    <li key={index} className="admin-product-img-item">
                      {file.name}
                    </li>
                  ))}
                </ul>
        ...
        {pageNumbers.map((pageNumber) => (
          <li key={pageNumber} className="admin-product-pagination-list">
            <button
              ...
              className={
                active === pageNumber
                  ? "admin-product-pagination-btn admin-product-active"
                  : "admin-product-pagination-btn"
              }
            >
              {pageNumber}
            </button>
