...
function AdminReview() {
      ...
      try {
        ...
        setReview(
          res.data.feedbacks.filter(
            (item) => item.User.name != "Nguyen Duc Phu",
          ),
        );
        setMainReview(
          res.data.feedbacks.filter(
  ...
  function handleActive(e) {
    ...
    if (id != 0) setMainReview(review.filter((r) => r.star === id));
    ...
    return (
      <table className="admin-review-table">
        <tr className="admin-review-row">
          <td className="admin-review-product_name">
    ...
    <div>
      ...
      <div className="admin-review-btn">
        <button
          className={
            currentActive === 0
              ? "admin-review-star-btn admin-review-active-btn"
              : "admin-review-star-btn"
          }
          id={0}
          onClick={handleActive}
        >
          Tất cả ({review.length})
        </button>
        <button
          className={
            currentActive === 1
          ...
          onClick={handleActive}
        >
          1 sao ({review.filter((r) => r.star === 1).length})
        </button>
        <button
          className={
            currentActive === 2
          ...
          onClick={handleActive}
        >
          2 sao ({review.filter((r) => r.star === 2).length})
        </button>
        <button
          className={
            currentActive === 3
          ...
          onClick={handleActive}
        >
          3 sao ({review.filter((r) => r.star === 3).length})
        </button>
        <button
          className={
            currentActive === 4
          ...
          onClick={handleActive}
        >
          4 sao ({review.filter((r) => r.star === 4).length})
        </button>
        <button
          className={
            currentActive === 5
          ...
          onClick={handleActive}
        >
          5 sao ({review.filter((r) => r.star === 5).length})
        </button>
      </div>
      <table className="admin-review-table">
        <tr className="admin-review-row">
        ...
        {pageNumbers.map((pageNumber) => (
          <li key={pageNumber} className="admin-review-pagination-list">
            <button
              ...
              className={
                active === pageNumber
                  ? "admin-review-pagination-btn admin-review-active"
                  : "admin-review-pagination-btn"
              }
            >
              {pageNumber}
            </button>
